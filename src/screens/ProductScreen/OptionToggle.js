...
const OptionToggle = ({onSelectOption, modifier}) => {
  const {id: modifierId, options, parentName, supportsNewDesign} = modifier || {};
  ...
  const customizationString = useMemo(() => {
    ...
    customizedModifiers?.forEach(cm => {
      ...
      const allSelectedNames = selections?.[id]?.selected?.map(item => item?.name);
